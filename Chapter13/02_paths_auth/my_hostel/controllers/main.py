...
class Main(http.Controller):
    ...
    def all_students_mark_mine(self):
        students = request.env['hostel.student'].sudo().search([])
        hotels = request.env['hostel.hostel'].sudo().search([('rector', '=', request.env.user.partner_id.id)])
        my_rooms = request.env['hostel.room'].sudo().search([('hostel_id', 'in', hotels.id)])
        html_result = '<html><body><ul>'
        for student in students:
            if student.id in my_rooms.student_ids.ids:
    ...
    def all_students_mine(self):
        hotels = request.env['hostel.hostel'].sudo().search([('rector', '=', request.env.user.partner_id.id)])
        students = request.env['hostel.room'].sudo().search([('hostel_id', 'in', hotels.id)]).student_ids
        html_result = '<html><body><ul>'
        for student in students:
            html_result += "<li> %s </li>" % student.name
        html_result += '</ul></body></html>'
